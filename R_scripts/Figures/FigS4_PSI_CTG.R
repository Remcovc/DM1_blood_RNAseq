...
A <- ggplot(df, aes(x = effect*100, y = -log10(p.value)))+
  geom_label_repel(label = ifelse(lm_fit_values[["V2Mode_SE"]][,"exon"] %in%  
                                    ...
                                    max.overlaps = 100, force = 25, color = "black", size = 6, fontface ="italic",
                                    segment.size = 0.25) +
...
ggsave(plot, 
       file ="Z:/Analysis_projects/DvA_recognition_rnaseq_diff_expr/Publication_GitHub/Revision updates/Figures/FigS4_CTG_PSI.png", 
       height = 13, width = 13, 
       dpi = 1200,
       device="png")
